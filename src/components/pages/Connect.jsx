  ...
  const handleSubmit = async e => { 
    ...
    try {
      await addContactSubmission({
        ...
        email: '',
        phone: form.mobile,
        subject: `Free Consultation Request - ${form.home}`,
        message: `Consultation request for ${form.home} property`,
        projectType: 'residential'
      });
      alert('Consultation request sent! We will contact you soon.');
      setForm({ name: '', mobile: '', home: '2BHK' });
    } catch (error) {
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20 gap-8 sm:gap-12 lg:gap-16">
        ...
        <form onSubmit={handleSubmit} className="flex-1 bg-white rounded-2xl sm:rounded-3xl shadow-2xl p-6 sm:p-8 lg:p-10 max-w-md w-full min-w-[280px] sm:min-w-[320px] flex flex-col gap-4 sm:gap-6 border border-gray-100">
          ...
          <button 
            type="submit"
            disabled={submitting}
            className="mt-2 sm:mt-4 w-full py-2 sm:py-3 rounded-full bg-yellow-500 hover:bg-yellow-600 text-white font-semibold text-base sm:text-lg shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
          >
